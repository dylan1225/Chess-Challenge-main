...
//old version of my bot so I can see how much better my cur bot is
namespace ChessChallenge.Example
{
    ...
    int Quiesce(Board board, int alpha, int beta){
        if(t_time.MillisecondsElapsedThisTurn > 4800) return 0;
    ...
    int NegMax(Board board, int alpha, int beta, int dep){
        if(t_time.MillisecondsElapsedThisTurn > 4800 || board.IsDraw()) return 0;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        foreach(Move move in legal_move){
            board.MakeMove(move);
            int score = -NegMax(board, -beta, -alpha, 2);
            board.UndoMove(move);
            if(score >= beta) break;
            if(score > alpha){
                alpha = score;
            ...
            if(timer.MillisecondsElapsedThisTurn > 4800) break;
        }
        return best_move;
    }
    }
